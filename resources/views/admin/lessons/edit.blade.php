        ...
        <form action="{{ route('admin.lessons.update', $lesson) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="duration" id="duration" value="{{ old('duration', $lesson->duration) }}" min="30" max="480"
                        class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:ring-blue-200"
                        required>
                    @error('duration')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price', $lesson->price) }}" min="0" step="0.01"
                        class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:ring-blue-200"
                        required>
                    @error('price')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="max_participants" id="max_participants" value="{{ old('max_participants', $lesson->max_participants) }}" min="1" max="20"
                        class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:ring-blue-200"
                        required>
                ...
                <div>
                    ...
                    <select name="difficulty_level" id="difficulty_level"
                        class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:ring-blue-200"
                        required>
                        <option value="beginner" {{ old('difficulty_level', $lesson->difficulty_level) == 'beginner' ? 'selected' : '' }}>Beginner</option>
                        <option value="intermediate" {{ old('difficulty_level', $lesson->difficulty_level) == 'intermediate' ? 'selected' : '' }}>Intermediate</option>
                        <option value="advanced" {{ old('difficulty_level', $lesson->difficulty_level) == 'advanced' ? 'selected' : '' }}>Advanced</option>
                        <option value="any" {{ old('difficulty_level', $lesson->difficulty_level) == 'any' ? 'selected' : '' }}>Any Level</option>
                    </select>
